...
Recorded by the implementer (TEAM-4151). No file outside the `## Files` table was touched.

| # | What changed | Why | Risk |
| --- | --- | --- | --- |
| 1 | CSS selector is `.settings__section .settings__control-meta`, not the flat `.settings__control-meta` the Approach specified. Declarations (`font-size: 0.75rem; color: #475569; line-height: 1.5; margin: 0.75rem 0 0`) are exactly as planned. | The plan's mitigation ("set `0.75rem` explicitly to win over `.settings__section p`") is insufficient: `.settings__section p` has specificity (0,1,1) and beats a lone class selector at (0,1,0). Verified in a real browser — with the flat selector the new line computed to **14px**, and the pre-existing `.settings__control-description` likewise computes to 14px today despite declaring `0.75rem` (a latent cascade bug in existing CSS, left untouched as out of scope). Scoping under `.settings__section` makes the spec's required 0.75rem/12px actually apply; browser-verified at 12px vs the description's 14px. | Low. Additive selector on a new class only; no existing rule or element is affected, and no new color/size/line-height value is introduced. |
| 2 | The R6 App test spies on `Storage.prototype.setItem` scoped to the `demo.settings.emailNotifications` key (other keys pass through to the real implementation), and a second test covers a fully unavailable `localStorage` (both `setItem` and `getItem` throwing) asserting only that the toggle still flips with no error text. | The plan's single R6 App test co-asserts "the settings write throws" and "`getActivities().length` incremented by 1". A global `setItem` throw also blocks `writeStoredActivities`, so the activity would not persist and those two assertions contradict each other. Key-scoping isolates the settings-write failure, which is what spec R6 ("the activity entry is still recorded once") actually describes; the added total-outage test preserves coverage of the storage-unavailable case. | Low. Test-only change; broadens R6 coverage rather than narrowing it. |
| 3 | Five tests were added beyond the plan's enumeration, all in files already listed in `## Files`: persisted-`false` renders Off *with* the line (R1/R2 distinction), the line is not an `aria-live` region (R3), the 30 s tick writes neither storage nor activity (R4 tick rule), the refresh interval leaves no pending timer after unmount (Plan Approval gate condition 2), and an out-of-range persisted timestamp degrades to R2 behavior (R5). | The plan's Approach asserts these behaviors but its Test plan did not pin them; gate condition (2) in particular asked for the unmount cleanup to be verifiable. | None. Additive assertions on planned behavior; no production code changed to accommodate them. |
